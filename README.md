...
**Create `config/jupyter.py`**
```
mkdir config
echo "" > config/jupyter.py
```
In `config/jupyter.py` add:
...
# Notebook config
c.NotebookApp.notebook_dir = 'mandirs'
c.NotebookApp.allow_origin = u'jupyter.herokuapp.com' # put your public IP Address here
c.NotebookApp.ip = '*'
...
- `c.NotebookApp.notebook_dir` I set as `mandirs` which means you should create a directory as `mandirs` for your default notebooks directory. In my case, jupyter will open right to this directory ignoring all others.
...
Test your new configuration locally with: `jupyter notebook --config=./config/jupyter.py`


#### 3.Create a notebook under -> `mandirs/Load_Unload.ipynb`
...
/usr/local/bin/jupyter notebook --config=./config/jupyter.py
```
...
##### 1. Create heroku app
```
heroku create jupyter
```
- Change `jupyter` to your app name
...
- On the commands above, you might have to append `-a <your-app-name>` like `heroku container:push web -a jupyter 
...
### Create a notebook under -> `mandirs/Load_Unload.ipynb`
